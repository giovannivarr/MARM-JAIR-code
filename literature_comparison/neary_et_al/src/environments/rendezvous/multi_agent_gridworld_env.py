...
class MultiAgentGridWorldEnv:
    ...
    def __init__(self, rm_file, num_agents, env_settings, invariant_experiment=False, strategy_rm=False):
        """
        ...
        self.invariant_experiment = invariant_experiment
        if invariant_experiment:
            self.discharged = False
            self.objects[(2, 2)] = 'charging_station'
    ...
    def environment_step(self, s, a):
        ...
        if self.invariant_experiment:
            ...
            if self.u == 1:
                ...
                if all(self.reached_goal_flag[i] for i in range(self.num_agents - 1)):
                    e = ','.join(['g{}'.format(i+1) for i in range(self.num_agents - 1)])
                    u2 = self.reward_machine.get_next_state(self.u, e)
                    r = r + self.reward_machine.get_reward(self.u, u2)
                    # Update the reward machine state
                    self.u = u2
        else:
            if self.strategy_rm and self.u == 1:
                for i in range(self.num_agents):
                    if 'g{}'.format(i + 1) in l:
                        self.reached_goal_flag[i] = True

                if all(self.reached_goal_flag[i] for i in range(self.num_agents)):
                    e = ','.join(['g{}'.format(i+1) for i in range(self.num_agents)])
                    u2 = self.reward_machine.get_next_state(self.u, e)
                    r = r + self.reward_machine.get_reward(self.u, u2)
                    # Update the reward machine state
                    self.u = u2
    ...
    def get_mdp_label(self, s, s_next, u):
        ...
        for i in range(self.num_agents):
            ...
            if i == 2 and self.invariant_experiment:
                row_next, col_next = self.get_state_description(s_next[i])
                if (row_next, col_next) in self.objects and (row, col) in self.objects:
                    if self.objects[(row, col)] == self.objects[(row_next, col_next)] == 'charging_station':
                        l.append('charged')
                continue
        ...
        if agent_all_on_wait:
            ...
            temp = l[:]
            l = ['r']
            if 'charged' in temp and self.invariant_experiment:
                l = ['r,charged']
            elif self.invariant_experiment:
                l = ['r,discharged']
    ...
    def get_options_list(self, agent_id):
        ...
        """
        if self.invariant_experiment and agent_id == 2:
            return ['recharge']
    ...
    def get_completed_options(self, s):
        ...
        for i in range(self.num_agents):
            ...
            if (row, col) in self.objects:
                ...
                elif self.objects[(row, col)] == 'charging_station' and i == 2 and self.invariant_experiment:
                    completed_options.append('recharge')
    ...
    def get_next_agent_option(self, agent_id, rm_state):
        ...
        if self.invariant_experiment and agent_id == 2:
            ...
            if rm_state == 0:
                return 'r{}'.format(agent_id + 1)
            elif rm_state == 1:
                return 'g{}'.format(agent_id + 1)
            else:
                return 'w{}'.format(agent_id + 1)
...
def play():
    ...
    rm_string = os.path.join(base_file_dir, 'reward_machine_files', 'rendezvous',
                             '{}_agent_rendezvous_rm.txt'.format(n))
