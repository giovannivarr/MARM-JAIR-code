...
class GridWorldEnv:
    ...
    def __init__(self, rm_file, agent_id, env_settings, strategy_rm=False, invariant_experiment=False):
        """
    ...
    def get_mdp_label(self, s, s_next, u):
        ...
        if not self.strategy_rm:
            ...
            if u == 0 and (row, col) in self.objects and (row_next, col_next) in self.objects:
                if self.objects[(row, col)] == self.objects[(row_next, col_next)] == 'w' \
                        and np.random.random() <= thresh:
                    l.append('r')
            elif u == 1:
                if (row_next, col_next) in self.objects:
                    if self.objects[(row_next, col_next)] == 'g':
    ...
    def get_train_strategy_label(self, s, s_next, u):
        """
        ...
        if self.invariant_experiment and self.agent_id == 3:
            if (row, col) in self.objects and (next_row, next_col) in self.objects:
                if self.objects[(row, col)] == self.objects[(next_row, next_col)] == 'charging_station':
                    return ['charged']
            elif np.random.random() <= 0.0125:
                return ['discharged']

        if (row, col) in self.objects and (next_row, next_col) in self.objects:
            if self.objects[(row, col)] == 'w' and self.objects[(next_row, next_col)] == 'w' \
                    and np.random.random() <= thresh:
                l = ['r']
            if self.invariant_experiment:
                if np.random.random() <= 0.165:
                    self.system_recharged = True


        if (next_row, next_col) in self.objects:
            if self.objects[(next_row, next_col)] == 'g':
                l.append('g{}'.format(self.agent_id))
    ...
    def hrm_environment_step(self, s, a, option=None):
        ...
        l = self.get_train_strategy_label(s, s_next, self.u)
        r = 0
...
def play():
    ...
    rm_string = os.path.join(base_file_dir, 'reward_machine_files', 'rendezvous',
                             'coordination_experiment_agent{}.txt'.format(agent_id))
