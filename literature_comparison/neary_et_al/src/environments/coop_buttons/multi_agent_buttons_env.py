...
class MultiAgentButtonsEnv:
    ...
    def __init__(self, rm_file, num_agents, env_settings, strategy_rm=False):
        """
    ...
    def get_next_state(self, s, a, agent_id):
        ...
        if agent_id == 0:
            if self.strategy_rm:
                if (self.u == 0) or (self.u == 1) or (self.u == 2):
...
def play():
    ...
    rm_string = os.path.join(base_file_dir, 'reward_machine_files', 'buttonsCopy', 'team_buttons_rm.txt')
