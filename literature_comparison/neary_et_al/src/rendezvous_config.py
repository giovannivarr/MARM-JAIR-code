...
def rendezvous_config(num_times, num_agents, strategy_rm=False):
    """
    ...
    local_rm_files = []
    if not strategy_rm:
        joint_rm_file = os.path.join(base_file_path, 'reward_machine_files', 'rendezvous',
                                     'team_rendezvous_rm.txt')
        for i in range(num_agents):
            local_rm_string = os.path.join(base_file_path, 'reward_machine_files', 'rendezvous',
                                           'rendezvous_rm_agent_{}.txt'.format(i + 1))
            local_rm_files.append(local_rm_string)
            
    else:
        joint_rm_file = os.path.join(base_file_path, 'reward_machine_files', 'rendezvous',
                                     'team_rendezvous_strategy.txt')
        for i in range(num_agents):
            local_rm_string = os.path.join(base_file_path, 'reward_machine_files', 'rendezvous',
                                           'rendezvous_strategy_agent_{}.txt'.format(i + 1))
            local_rm_files.append(local_rm_string)
