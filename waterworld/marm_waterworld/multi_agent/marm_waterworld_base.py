...
coord_fixed_food = [375, 375]

class MARMWaterworldBase:
    def __init__(
            self,
            reward_machines,
            n_pursuers=2,
        ...
        reward_machines: reward machines of the agents, the i-th RM corresponds to the i-th agent
        ...
        rm_less: whether to not include the state of the RM in the observation
        """
        ...
        for rm in reward_machines:
            # Loading the RMs, the i-th RM corresponds to the i-th agent
            if reward_shaping:
                rm.add_reward_shaping(gamma=0.99, rs_gamma=0.9)
            self.num_rm_states.append(len(rm.get_states()))
            self.reward_machines.append(copy(rm))
    ...
    def get_spaces(self):
        ...
        for i in range(self.n_pursuers):
            # Increase the shape size by the length of the one-hot encoding of the RM of pursuer i, if the agent can observe the RM
            obs_dim_shape = obs_dim + self.num_rm_states[i] if not self.rm_less else obs_dim
    ...
    def step(self, action, agent_id, is_last):
        ...
        if is_last:
            ...
            obs_list = self.observe_list()
            self.last_obs = [obs_list[id][0] for id in range(self.n_pursuers)]
    ...
    def observe_list(self):
        ...
        for i, pursuer in enumerate(self.pursuers):
            ...
            rm_rew = 0
            if self.collisions != set():
                # print(i, 'current rm state:', self.reward_machines[i].current_state, "&".join(self.collisions))
                # print("&".join(self.collisions))
                ...
                # print(rm_rew)
                # print(i, 'current rm state:', self.reward_machines[i].current_state)
    ...
    def pursuer_evader_begin_callback(self, arbiter, space, data):
        ...
        # Add collision to set of collisions
    ...
    def pursuer_evader_separate_callback(self, arbiter, space, data):
        ...
        # Remove collision from set of collisions
        ...
        if evader_shape.counter < evader_shape.n_coop:
            ...
            # Do not reset the evaders when the collision is finished
