...
| :--------------------------------: | -------------------------------------------- | :---------: |
|           0 to n_sensors           | Barrier distance for each sensor             |   [0, 1]   |
|  n_sensors to (2 * n_sensors)      | Food distance for each sensor                |   [0, 1]   |
| (2 * n_sensors) to (3 * n_sensors) | Food speed for each sensor                   | [-2*√2, 2*√2] |
|           3 * n_sensors            | Indicates whether agent collided with food   |   {0, 1}    |
...
|           0 to n_sensors          | Barrier distance for each sensor             | [0, 1] |
|    n_sensors to (2 * n_sensors)   | Food distance for each sensor                | [0, 1] |
|          2 * n_sensors            | Indicates whether agent collided with food   | {0, 1} |
...
from marm_waterworld.single_agent.sarm_waterworld_base import FPS
from marm_waterworld.single_agent.sarm_waterworld_base import SARMWaterworldBase as _env
...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(self, agent_id, reward_shaping, seed=None, *args, **kwargs):
        EzPickle.__init__(self, *args, **kwargs)
        AECEnv.__init__(self)
        self.env = _env(agent_id=agent_id, reward_shaping=reward_shaping, *args, **kwargs)

        self.agents = ["pursuer_{}".format(agent_id)]
        self.possible_agents = self.agents[:]
        self.agent_name_mapping = {self.agents[0]: agent_id}
    ...
    def step(self, action):
        ...
        self.env.step(action, is_last)

        for r in self.rewards:
            self.rewards[r] = self.env.control_rewards[-1]
        if is_last:
            for r in self.rewards:
                self.rewards[r] += self.env.last_rewards[-1]
