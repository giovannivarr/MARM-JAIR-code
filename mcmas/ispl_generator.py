...
if __name__ == "__main__":
    ...
    # If there's a third section, the third line specifies the extra variables in the ispl.
    env_vars = []
    if sections[3].split('\n')[3]:
    ...
    # If there is a forth line in the third section, then this (and the following ones) will specify how variables change at each timestep, 
    # if the agents do not modify them via their actions
    if sections[3].split('\n')[4]:
        for var in sections[3].split('\n')[4:]:
            prop = var.split(':')[0]
    ...
    for element in prop_symbols:
        if element not in ['U', 'X', 'F', 'G', 'and', 'or', '!', '(', ')', 'LTL']:
    ...
    if len(sections) == 4:
        # First line specifies the coalition we would like to consider when evaluating the ATL formula
        if sections[3].split('\n')[1].split(',')[0]:
            group = sections[3].split('\n')[1].split(',')

        # Second line in third section gives the initial truth values of the variables. If it's not specified, all variables are false
        # (For the third line, go back when we defined "env_vars")
        if sections[3].split('\n')[2].split(',')[0]:
    ...
    out_ispl += '\nFormulae\n\t<gA> ' + formula + ' ;\nend Formulae' if 'LTL' not in formula else '\nFormulae\n\t' + formula + ' ;\nend Formulae'
